...
class DataRetriever:
    ...
    def get_civ_info(self, civ_name):
        ...
        # Extract bonuses and unique items from civ data if available
        if civ_data:
            result['bonuses'] = civ_data.get('bons', [])
            result['unique_units'] = civ_data.get('unique', {}).get('units', []) if 'unique' in civ_data else []
            result['unique_techs'] = civ_data.get('unique', {}).get('techs', []) if 'unique' in civ_data else []
            result['team_bonus'] = civ_data.get('team_bonus', '')
